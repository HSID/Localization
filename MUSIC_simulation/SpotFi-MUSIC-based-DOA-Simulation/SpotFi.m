function varargout = SpotFi(varargin)
...
% Last Modified by GUIDE v2.5 11-Apr-2016 14:43:00
...
tof1 = str2double(get(hObject, 'String'))
handles.metricdata.tof1 = tof1;
guidata(hObject, handles)
...
tof2 = str2double(get(hObject, 'String'))
handles.metricdata.tof2 = tof2;
guidata(hObject, handles)
...
tof3 = str2double(get(hObject, 'String'))
handles.metricdata.tof3 = tof3;
guidata(hObject, handles)
...
function matrixOutput = pushbutton1_Callback(hObject, eventdata, handles)
...
%tof1 = handles.metricdata.tof1;
%tof2 = handles.metricdata.tof2;
%tof3 = handles.metricdata.tof3;
...
%doas=[th1 th2 th3]; %DOAs of signals in rad.
%tofs = [tof1, tof2, tof3]; %ToFs of signals in terms of sample period.
%P=[p1 p2 p3];
numberOfPathes=length(doas);
...
%end
X = generateSignalMatrix(doas, tofs, P, N, d, false);
%HeatMap(abs(X));
[music_spectrum, angles, TOFS] = computeMUSICSpectrum(X, numberOfPathes, d, true);
HeatMap(music_spectrum);
